...
function groupMediaQueries(mediaQueries, prefix = '') {
  const grouped = Object.entries(mediaQueries).reduce((result, [className, mediaQuery]) => {
  ...
  return Object.entries(grouped).map(([mediaQuery, selectors]) => `${selectors.join(', ')} ${mediaQuery}`);
}
