...
function collapseWhitespace(tree, options, collapseType, parent) {
    ...
    tree.forEach((node, index)=>{
        ...
        if (typeof node === 'string') {
            ...
            var _node_content;
            const isAllowCollapseWhitespace = !noWhitespaceCollapseElements.has(node.tag) && !hasWhitespacePreservingStyle(node);
 ...
 */ function hasWhitespacePreservingStyle(node) {
    var _node_attrs;
    const style = (_node_attrs = node.attrs) == null ? void 0 : _node_attrs.style;
    return typeof style === 'string' && whitespacePreservingStylePattern.test(style);
}
function isTrimmableAroundNode(node) {
    if (!node) return true;
    if (typeof node === 'string') return helpers_js.isComment(node);
    return typeof node.tag === 'string' && !noTrimWhitespacesArroundElements.has(node.tag);
}
function isCommentNode(node) {
    return typeof node === 'string' && helpers_js.isComment(node);
}
const mod = {
